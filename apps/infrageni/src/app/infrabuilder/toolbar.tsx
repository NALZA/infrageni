...
export const Toolbar: React.FC<ToolbarProps> = ({
    ...
    const toolbarButton = (active: boolean, onClick: () => void, label: string) => (
        <button
            className={`px-4 py-2 glass-button glass-button-hover transition-all duration-200 text-sm font-medium ${active
    ...
    const actionButton = (onClick: () => void, label: string, variant: 'primary' | 'secondary' = 'primary') => (
        <button
            className={`px-4 py-2 glass-button glass-button-hover transition-all duration-200 text-sm font-medium ${variant === 'primary'
    ...
    return (
        <div className="absolute top-0 left-0 w-full z-20 backdrop-blur-lg border-b border-white/10 dark:border-white/5 rounded-t-lg px-6 py-3" style={{ minHeight: 56 }}>
