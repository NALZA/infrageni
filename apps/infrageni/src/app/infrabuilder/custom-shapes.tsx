...
import { GenericComponent, GENERIC_COMPONENTS, useProvider } from './components';

// Helper function to get provider-specific label
function getProviderSpecificLabel(componentId: string, provider: string): string {
...
function ShapeContent({ componentId, icon, color }: { componentId: string; icon: React.ReactNode; color: string }) {
...
export type ComputeShape = TLBaseShape<
    'compute',
    {
        ...
        color: string;
        label: string;
        componentId: string; // Store the component ID to look up provider-specific names
    }
>;
